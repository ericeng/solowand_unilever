...
static int stream_error( char* error )
{
    ERRNO( error );
    wandeye_close();
    return -1;
...
int main(int argc, char* argv[])
{
    ...
    opterr = 0;
    while ((c = getopt(argc, argv, "a:c:dhlm:p:qr:s:t:")) != -1)
    {
        switch (c)
        {
	    ...
	    case 'r': /* frame rate */
		frame_rate = strtol( optarg, &err, 10 );
		break;
    ...
    {
	return stream_error( "sigaction() failed" );
    }
    ...
    { 
	return stream_error( "signal() failed" );
    }
    ...
    {
	return stream_error( "pthread_cond_init() failed" );
    }

    if (pthread_mutex_init(&jpeg_mutex, NULL))
    {
        return stream_error( "pthread_mutex_init() failed" );
    }
    ...
    if (socket_fd < 0)
    {
        return stream_error( "failed to start server" );
    }
    ...
    while (running)
    {
	    ...
	    return stream_error( "input: read() failed" );
        }
        else if (size == 0)
        {
        ...
        {
            return stream_error( "pthread_mutex_lock() failed" );
        }
	...
	wandeye();
        ...
        if (sep)
        { /* found a separator, jpeg frame is ready */
            if (auto_separator)
            {
            ...
            {
                return stream_error( "pthread_cond_broadcast() failed" );
            }
        ...
        {
            return stream_error( "pthread_mutex_unlock() failed" );
        }
		...
		{
                    return stream_error( "pthread_create() failed" );
                }
    ...
    {
        return stream_error( "pthread_mutex_destroy() failed" );
    }
    ...
    {
        return stream_error( "pthread_cond_destroy() failed" );
    }
